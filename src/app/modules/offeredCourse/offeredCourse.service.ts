...
const getAllOfferedCoursesFromDB = async (query: Record<string, unknown>) => {
  const offeredCourseQuery = new QueryBuilder(OfferedCourse.find().populate('semesterRegistration academicSemester academicFaculty academicDepartment course faculty'), query)
    .filter()
    .sort()
    .paginate()
    .fieldLimiting();
...
const getMyOfferedCoursesFromDB = async (
  ...
  if (!currentOngoingRegistrationSemester) {
    throw new AppError(
      ...
      'There is no ongoing registration semester',
    );
  }
  ...
  const totalDoc = (await OfferedCourse.aggregate(aggregationQuery)).length;
...
const updateOfferedCourseIntoDB = async (
  ...
  // Extract semester registration _id from the offered course
  const semesterRegistration = isOfferedCourseExists.semesterRegistration;

  // Only allow update if the semester registration is upcoming
  ...
  if (semesterRegistrationStatus?.status !== 'UPCOMING') {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      `You can not update the offered course when semester registration is ${semesterRegistrationStatus?.status}`,
    );
  }
  // Get the schedules of the faculties
  const assignedSchedule = await OfferedCourse.find({
...
const deleteOfferedCourseFromDB = async (id: string) => {
  ...
  // Only allow delete if the semester registration is upcoming
  const semesterRegistrationStatus =
    await SemesterRegistration.findById(semesterRegistration);
  ...
  if (semesterRegistrationStatus?.status !== 'UPCOMING') {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      `You can not delete the offered course when semester registration is ${semesterRegistrationStatus?.status}`,
    );
  }
